    ...
    public class UIManager : MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            if (!instance)
            {
                ...
                if (!Nav.Initalized)
                {
                    //var nav = instance.gameObject.GetComponent<NavProvider>();
                    //if (!nav)
                    //    nav = instance.gameObject.AddComponent<NavProvider>();
                    Nav.Initialize();
                    Nav.Current.Routes.Add("default", new Route(NavRouteHandler.DefaultPattern, new NavRouteHandler()));
                }
            }
        }
        ...
        {
            var dic = Route.ObjectToDictionary(parameter)??new Dictionary<string, object>();
            dic["__layer"] = layer;
